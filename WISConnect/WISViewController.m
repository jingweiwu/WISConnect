...
//    }
    ...
    if([self.currentSelectedOperation isEqualToString:@"更新当前用户详细信息"]) {
        [self.dataManager submitUserDetailInfoWithNewInfo:[self.dataManager currentUser] completionHandler:^(BOOL completedWithNoError, NSError *error, NSString *classNameOfDataAsString, id data) {
            if (completedWithNoError) {
                WISUser *user = data;
                user = user;
            }
